// Financial flow repair: always read authoritative KYC data from Supabase before appraisal and supervisor review.
(()=>{
 ...
 document.addEventListener('submit',async e=>{
  if(e.target.id!=='appraisalForm')return;e.preventDefault();e.stopImmediatePropagation();
  try{
   ...
   if(window.refresh)await window.refresh();show('applications',false);
 ...
 window.openReview=async id=>{
  try{
   ...
   const local=(window.apps||[]).find(x=>String(x.id)===String(id));if(local){Object.assign(local,a);local.customers=a.customers}
